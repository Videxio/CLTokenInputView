...
- (id)initWithToken:(CLToken *)token font:(nullable UIFont *)font
{
    ...
    if (self) {
        UIColor *tintColor = [UIColor colorWithRed:0.0823f green:0.4941f blue:0.9843f alpha:1.0f];
...
- (CGSize)intrinsicContentSize
{
    ...
    return CGSizeMake(labelIntrinsicSize.width+(2.0f*PADDING_X), labelIntrinsicSize.height+(2.0f*PADDING_Y));
}

- (CGSize)sizeThatFits:(CGSize)size
{
    CGSize fittingSize = CGSizeMake(size.width-(2.0f*PADDING_X), size.height-(2.0f*PADDING_Y));
    CGSize labelSize = [self.selectedLabel sizeThatFits:fittingSize];
    return CGSizeMake(labelSize.width+(2.0f*PADDING_X), labelSize.height+(2.0f*PADDING_Y));
}
